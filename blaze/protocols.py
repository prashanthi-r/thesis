...
class protocols:
	...
	def multiplication(a,b):
		...
		# Step 6
			gamma_xy = np.add(np.multiply(a.x3,(b.x1)),np.add(np.multiply(b.x3,(a.x1)),np.subtract(Psi[conf.partyNum],chi)))
		...
		if(conf.PRIMARY):
			my_beta_z = np.uint64(np.subtract(np.subtract(np.multiply(np.uint64(conf.partyNum - 1),np.multiply((a.x2),(b.x2))),np.multiply((a.x2),(b.x1))),np.add(np.multiply((b.x2),(a.x1)),np.add(gamma_xy,alpha))))

			adv_beta_z = prim.send_recv_val(my_beta_z,conf.adv_party)
			beta_z = np.add(my_beta_z,adv_beta_z)

		# Step 2
		if(conf.partyNum == 0):
			gamma_xy = np.multiply(np.add(a.x1,a.x2),np.add(b.x1,b.x2))
			beta_zstar = np.uint64(np.subtract(np.multiply(np.uint64(-1),np.multiply((a.x3),np.add(b.x1,b.x2))),np.add(np.add(np.multiply((b.x3),np.add(a.x1,a.x2)),alpha),np.add(np.multiply(2,(gamma_xy)),chi))))
		...
		if(conf.PRIMARY):
			...
			if(conf.partyNum == 1):
				...
				prim.send_val(np.add(beta_z,gamma),0)
			else:
